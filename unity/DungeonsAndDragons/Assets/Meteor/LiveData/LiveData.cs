		...
		void HandleOnTextMessageRecv (string socketMessage)
		{
			...
			case AddedMessage.added:
				...
				if (Collections.Contains(collection))
				{
					...
					Debug.LogError(string.Format("LiveData: Unhandled record add. Creating a collection to handle it.\nMessage:\n{0}",socketMessage));
				}
				break;
			case ChangedMessage.changed:
				...
				if (Collections.Contains(cm.collection))
				{
					...
					Debug.LogError(string.Format("LiveData: Unhandled record change. Cannot recover this record later.\nMessage:\n{0}",socketMessage));
				}
				break;
			case RemovedMessage.removed:
				...
				if (Collections.Contains(rm.collection))
				{
					...
					Debug.LogError(string.Format("LiveData: Unhandled record remove.\nMessage:\n{0}",socketMessage));
				}
				break;
			...
			default:
				if (!socketMessage.Contains("server_id")) {
					Debug.LogError(string.Format("LiveData: Unhandled message.\nMessage:\n{0}",socketMessage));
				}
				break;
			}
		}
